...
file.opts<-list(
  prefix=prefix,
  filename=paste(prefix,"",sep=""),
  metadata.filename=paste(prefix,"_Map",sep=""),
  group.type="status",
  
  pred.prefix=pred.prefix,
  pred.filename=paste(pred.prefix,"",sep=""), #.Species.classified
  pred.metadata.filename=paste(pred.prefix,"_Map",sep=""),
  pred.group.type="status",
  fdr_cutoff=0.1,
  rf.cv=TRUE
)
#--------------------------------------------------
rf.opts<-list(outdir=NA,ntree=5000,verbose=FALSE,errortype="oob",n.oob=10,nfolds=10)
...
# Training data input
#-------------------------------
g<-read.csv(paste(filename,".csv",sep=""),header=T,row.names=1)
...
#-------------------------------
metadata<-read.table(paste(metadata.filename,".txt",sep=""),header=T,sep="\t",row.names=1)
...
if(!is.na(file.opts$pred.prefix)){
  ...
  # Test data input
  #-------------------------------
  pred.g<-read.csv(paste(pred.filename,".csv",sep=""),header=T,row.names=1)
  ...
  #-------------------------------
  pred.metadata<-read.table(paste(pred.metadata.filename,".txt",sep=""),header=T,sep="\t",row.names=1)
  ...
  if(nrow(wilcoxEnr)>0){
    ...
    if(!is.na(file.opts$pred.prefix)){
      ...
      p <- ggplot(data=data_melt,aes(x=variable,y=value,fill=Group))+xlab("Taxa")+ylab("Relative abundance (log10 scale)")+
        ...
        scale_fill_manual(name = file.opts$group.type, values = MyColor)+
        coord_flip()+ #if want to filp coordinate
        geom_jitter(position=position_jitterdodge(jitter.width= 0.2,dodge.width = 0.8),size=1,aes(fill=Group),alpha=0.4)  #jitter
